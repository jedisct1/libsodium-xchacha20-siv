...
static void
s2v(unsigned char iv[crypto_aead_det_xchacha20_MACBYTES], const unsigned char *m, size_t mlen,
    const unsigned char *ad, size_t adlen, const unsigned char *nonce, size_t noncelen,
    const unsigned char ka[32])
{
    static const unsigned char zero[crypto_aead_det_xchacha20_MACBYTES] = { 0 };
    ...
    if (ad != NULL && adlen > 0) {
        s2v_dbl256(d);
        crypto_generichash(iv, crypto_aead_det_xchacha20_MACBYTES, ad, adlen, ka, 32);
        s2v_xor(d, iv, sizeof d);
    }
    if (nonce != NULL && noncelen > 0) {
        s2v_dbl256(d);
        crypto_generichash(iv, crypto_aead_det_xchacha20_MACBYTES, nonce, noncelen, ka, 32);
        s2v_xor(d, iv, sizeof d);
    }

    crypto_generichash_init(&st, ka, 32, crypto_aead_det_xchacha20_MACBYTES);
    if (mlen >= crypto_aead_det_xchacha20_MACBYTES) {
        crypto_generichash_update(&st, m, mlen - crypto_aead_det_xchacha20_MACBYTES);
        s2v_xor(d, &m[mlen - crypto_aead_det_xchacha20_MACBYTES],
    ...
    crypto_generichash_final(&st, iv, crypto_aead_det_xchacha20_MACBYTES);
}
...
{
    unsigned char subkeys[64], *ka = &subkeys[0], *ke = &subkeys[32];
    unsigned char computed_mac[crypto_aead_det_xchacha20_MACBYTES];
    ...
    if (sodium_memcmp(mac, computed_mac, crypto_aead_det_xchacha20_MACBYTES) != 0) {
        memset(m, 0, mlen);
        return -1;
    }
    return 0;
                                  ...
                                  const unsigned char k[crypto_aead_det_xchacha20_KEYBYTES])
{
    ...
    if (clen < crypto_aead_det_xchacha20_MACBYTES) {
        return -1;
    }
    mlen = clen - crypto_aead_det_xchacha20_MACBYTES;
